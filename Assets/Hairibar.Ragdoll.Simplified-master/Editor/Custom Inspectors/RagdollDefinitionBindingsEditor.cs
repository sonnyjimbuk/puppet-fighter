    ...
    internal class RagdollDefinitionBindingsEditor : UnityEditor.Editor
    {
        ...
        void ValidateNoDuplicateJoints()
        {
            ...
            for (int i = 0; i < values.arraySize; i++)
            {
                ...
                if (alreadyPresent)
                {
                    EditorGUILayout.HelpBox("The same Joint is bound to multiple bones. This is not allowed.", MessageType.Error);
                    return;
                }
            }
        }
        ...
        void ValidateNoUnassignedBones()
        {
            for (int i = 0; i < values.arraySize; i++)
            {
                ...
                {
                    EditorGUILayout.HelpBox("All bones must be bound to a ConfigurableJoint.", MessageType.Error);
                    return;
                }
            }
        }
