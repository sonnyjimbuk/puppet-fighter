...
namespace Hairibar.Ragdoll.Animation.Editor
{
    ...
    internal class RagdollAnimationProfileEditor : RagdollProfileEditor
    {
        ...
        protected override void DrawInspector()
        {
            ...
            {
                EditorGUILayout.HelpBox("You can assign a definition to be able to override individual bones.", MessageType.Info);
            }
        ...
        void DrawOverrideList(ReorderableList list)
        {
            ...
            {
                EditorGUILayout.HelpBox("Override editing is not supported when editing multiple profiles.", MessageType.Info);
            }
            else
            {
                list.DoLayoutList();
        ...
        void ValidateNoDuplicateOverrides(ReorderableList list)
        {
            ...
            for (int i = 0; i < list.serializedProperty.arraySize; i++)
            {
                ...
                {
                    EditorGUILayout.HelpBox($"Bone \"{bone}\" is duplicated.", MessageType.Warning);
        ...
        void WarnAgainstUnstablePositionMatchingParameters()
        {
            ...
            {
                EditorGUILayout.HelpBox($"Settings where (alpha + dampingRatio > {MAX_STABLE_ALPHA_PLUS_DAMPING}) are often unstable. Consider lowering the parameters."
                    , MessageType.Warning);
        ...
        void WarnAgainstPositionWithoutRotation()
        {
            ...
            if (hasInvalidBones)
            {
                EditorGUILayout.HelpBox("Position Matching with no Rotation Matching can produce horrifying results. \n" +
                    "To avoid this, make sure that no bone has a Position Alpha over 0 and a Rotation Alpha of 0.", MessageType.Warning);
            }
        }
        #endregion
