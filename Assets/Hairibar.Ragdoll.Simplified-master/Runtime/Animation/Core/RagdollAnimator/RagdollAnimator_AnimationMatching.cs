    ...
    public partial class RagdollAnimator
    {
        ...
        void DoPoweredPositionMatching(AnimatedPair pair, BoneProfile boneProfile, float dt)
        {
            ...
            Vector3 acceleration = AnimationMatching.GetAccelerationFromPositionSpring(rigidbody.position, targetPose.worldPosition,
                rigidbody.linearVelocity, pair.poseLinearVelocity, alpha, dampingRatio, rigidbody.mass, dt);
        ...
        {
            // So. With k = 0, the joint applies forces to match target angular velocity.
            // These forces are limited (?????) by damping AND by max forces. WHAT. 
            // Also, the velocity needs to be INVERTED. So, target = -velocity.
            // With k > 0, the joint uses a spring to get to the target rotation.
            // In this case, target velocity is ignored for applying forces, but it is used for damping (so, relative velocity damping).
